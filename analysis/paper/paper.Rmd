...
# Summary of methods
Here, we directly compare two large collections of radiocarbon and tree-ring dates drawn from the Upland US Southwest. 
...
The UUSW was geographically definied by @Bocinsky2016_SA as the region between 105--113ºW and 32--38ºN. Here, we refine that definition in two ways. First, since location data for our radiocarbon database are only resolved to the county level, we define the study region to include all counties whose geographic centroids fall within the region from @Bocinsky2016_SA. Second, we roughly exclude the Phoenix and Tucson basins by excluding Maricopa, Pinal, and Pima counties in Arizona. Figure \@ref(fig:study-area) shows the original UUSW from @Bocinsky2016_SA, and the revised study area used in this analysis.
...
ggplot2::ggplot() +
  ...
  ggplot2::geom_sf(data = four_corners_states,
                   fill = NA) +
  ggplot2::geom_sf(data = UUSW,
                   fill = NA) +
    ggplot2::geom_sf(data = UUSW_counties,
                   fill = NA) +
